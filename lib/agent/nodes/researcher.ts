...
const RESEARCHER_MODEL = process.env.RESEARCHER_MODEL || process.env.GEMMA_MODEL || "gemini-2.5-flash";
...
export async function researcherNode(state: AgentStateType): Promise<Partial<AgentStateType>> {
  ...
  const isGroq = RESEARCHER_MODEL.toLowerCase().includes("llama") || 
                  RESEARCHER_MODEL.toLowerCase().includes("groq") || 
                  RESEARCHER_MODEL.toLowerCase().includes("mixtral");
  ...
  const toolResults = await Promise.all(
    toolCalls.map(async (call) => {
      ...
      const localLogs: string[] = [`[CALL] ${toolName}: ${JSON.stringify(args)}`];
      let result = "";
      let detail = "";
      ...
      try {
        ...
        localLogs.push(`[SUCCESS] ${toolName} - ${detail}`);
      } catch (err) {
        const errMsg = err instanceof Error ? err.message : String(err);
        result = JSON.stringify({ error: errMsg });
        localLogs.push(`[ERROR] ${toolName} failed: ${errMsg}`);
      }
  ...
  for (const tr of toolResults) {
    log.push(...tr.localLogs);

    notes.push({
      tool: tr.toolName,
      query: JSON.stringify(tr.args),
      result: tr.result,
      timestamp: new Date().toISOString(),
    });

    if (tr.toolName === "company_lookup") {
      try {
        const parsed = JSON.parse(tr.result);
        if (parsed.ticker) {
          updatedTicker = parsed.ticker;
        } else if (parsed.error === "NOT_FOUND") {
          stateError = tr.result;
    ...
    toolsUsedThisRound.push(tr.toolName);
  }
